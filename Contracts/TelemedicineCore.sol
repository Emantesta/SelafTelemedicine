...
struct Patient {
    ...
    uint48 registrationTimestamp; // Renamed for clarity
...
function initialize(
    ...
    dataMonetizationReward = 10 * 10**18; // 10 points (assuming 18 decimals for consistency)
    aiAnalysisCost = 0.01 ether;
    pointsPerLevel = 100;
    maxLevel = 10;
    decayRate = 10; // 10% decay
    decayPeriod = 30 days;
    freeAnalysisPeriod = 30 days;
    minReserveBalance = RESERVE_FUND_THRESHOLD;
    versionNumber = 1;

    discountLevels[3] = 5;  // 5% discount at level 3
    discountLevels[5] = 10; // 10% discount at level 5
...
// Dispute Resolution Setter
function setDisputeResolution(address _disputeResolution) external onlyRole(ADMIN_ROLE) {
    require(_disputeResolution != address(0), "Dispute resolution address cannot be zero");
    address oldAddress = disputeResolution;
...
function toggleDataMonetization(bool _enable) external onlyRole(PATIENT_ROLE) nonReentrant whenNotPaused {
    ...
    if (_enable && patient.dataSharing == DataSharingStatus.Disabled) {
        ...
        patient.lastActivityTimestamp = uint48(block.timestamp); // Reset for reward tracking
...
function decayPoints(address _patient) external nonReentrant whenNotPaused {
    require(hasRole(PATIENT_ROLE, _patient), "Not a patient");
...
function claimFreeAnalysis() external onlyRole(PATIENT_ROLE) nonReentrant whenNotPaused {
    Patient storage patient = patients[msg.sender];
    require(patient.isRegistered, "Patient not registered");
    require(block.timestamp >= patient.lastFreeAnalysisTimestamp.add(freeAnalysisPeriod), "Free analysis not available yet");
    require(aiAnalysisFund >= aiAnalysisCost, "Insufficient AI fund");
...
function _claimDataReward(address _patient) internal {
    ...
    require(block.timestamp > patient.lastActivityTimestamp, "No reward accrued");

    uint256 timeElapsed = block.timestamp.sub(patient.lastActivityTimestamp);
    uint256 reward = timeElapsed.mul(dataMonetizationReward).div(1 days); // Reward scaled by time
...
// Public Getter for Paused State (required by TelemedicineDisputeResolution)
