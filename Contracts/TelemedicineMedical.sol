<xaiArtifact artifact_id="b46fa4b2-d47e-40e1-8859-a4dad2d7adda" artifact_version_id="bea74dfe-69d9-4e87-aa7a-4956e88be3a9" title="TelemedicineMedical.sol" contentType="text/solidity">
...
function bookAppointment(
    ...
    _addPendingAppointment(_doctor, appointmentCounter);
    ...
    emit AppointmentBooked(appointmentCounter, msg.sender, _doctor, _timestamp, _videoCallLink);
}
...
function rescheduleAppointment(uint256 _appointmentId, uint256 _newTimestamp) external onlyRole(core.PATIENT_ROLE()) nonReentrant whenNotPaused {
    Appointment storage apt = appointments[_appointmentId];
    require(apt.patient == msg.sender, "Not your appointment");
    ...
    apt.scheduledTimestamp = uint48(_newTimestamp);
...
function completeAppointment(uint256 _appointmentId, string calldata _ipfsSummary) external onlyRole(core.DOCTOR_ROLE()) whenNotPaused {
    Appointment storage apt = appointments[_appointmentId];
    require(apt.doctor == msg.sender, "Not your appointment");
...
function _monetizeData(address _patient) internal {
    ...
    if (patient.dataSharing == TelemedicineCore.DataSharingStatus.Enabled && block.timestamp >= patient.lastRewardTimestamp.add(1 days)) {
        if (payments.sonicToken().balanceOf(address(payments)) >= core.dataMonetizationReward()) {
            patient.lastRewardTimestamp = uint48(block.timestamp);
            require(payments.sonicToken().transfer(_patient, core.dataMonetizationReward()), "SONIC transfer failed");
            emit DataRewardClaimed(_patient, core.dataMonetizationReward());
        }
    }
}
