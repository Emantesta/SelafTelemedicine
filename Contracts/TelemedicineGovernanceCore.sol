...
// TimeLock Functions
function _queueTimeLock(TimeLockAction _action, address _target, uint256 _value, bytes memory _data) public onlyRole(core.ADMIN_ROLE()) returns (uint256) {
...
function executeTimeLock(uint256 _timeLockId) external onlyRole(core.ADMIN_ROLE()) nonReentrant whenNotPaused {
    ...
    if (tl.action == TimeLockAction.AddAdmin) {
        ...
        // Dispute resolution execution is handled by the target contract (TelemedicineDisputeResolution)
    }
